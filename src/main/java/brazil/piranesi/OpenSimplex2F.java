...
public class OpenSimplex2F {
	 ...
	 * @param freq 
	 */
	public double noise2(double x, double y, int freq) {
		
		// Get points for A2* lattice
		double s = 0.366025403784439 * (x + y);
		double xs = x + s, ys = y + s;
		
		return noise2_Base(xs/freq, ys/freq);
	}
	 ...
	 */
	public double noise2_XBeforeY(double x, double y, int frequency) {
		
		// Skew transform and rotation baked into one.
		double xx = x * 0.7071067811865476 / frequency;
		double yy = y * 1.224744871380249 / frequency;
	 ...
	 */
	public double noise3_XYBeforeZ(double x, double y, double z, double freq) {
		...
		// Evaluate both lattices to form a BCC lattice.
		return noise3_BCC(xr, yr, zr) / (freq * freq);
	}
	 ...
	 */
	public double noise3_XZBeforeY(double x, double y, double z, int freq) {
		
		// Re-orient the cubic lattices without skewing, to make X and Z triangular like 2D.
		// Orthonormal rotation. Not a skew transform.
		double xz = x + z;
		double s2 = xz * -0.211324865405187 / (2*freq);
		double yy = y * 0.577350269189626 / (2*freq);
		double xr = x + s2 - yy; double zr = z + s2 - yy;
		double yr = xz * 0.577350269189626 + yy / (5*freq);
		
		// Evaluate both lattices to form a BCC lattice.
		return noise3_BCC(xr/freq, yr/freq, zr/freq);
	}
